<template>
  <div id="app" v-cloak>
    ...
    <div class="Camera-wrapper" :style="{ width: width + 7 + 'px', height: height + 7 + 'px' }">
      ...
      <van-circle v-model="currentRate" :style="{ width: width + 7 + 'px', height: height + 7 + 'px' }" :rate="0"
                  :speed="100" stroke-width="20" layer-color="#ebedf0"></van-circle>
    </div>
    ...
    <div class="Camera-toggle" title="切换摄像头" @click="actionShow = true">
      <svg t="1658028836694" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
        ...
        <path d="M436.876 473.188c-17.257 0-31.249 13.987-31.249 31.249v30.486c0 17.262 13.992 31.249 31.249 31.249s31.249-13.987 31.249-31.249v-30.486c0-17.261-13.992-31.249-31.249-31.249z"
              p-id="5843" fill="#ffffff"></path>
        <path d="M587.024 473.188c-17.262 0-31.249 13.987-31.249 31.249v30.486c0 17.262 13.987 31.249 31.249 31.249s31.249-13.987 31.249-31.249v-30.486c0-17.261-13.987-31.249-31.249-31.249z"
...
export default {
  ...
  data() {
    return {
      ...
      currentRate: 0,
      nextactionTime: 0,
      isOpenMouthDetected: false,
      isShakingHeadDetected: false,
      isLookingUpDetected: false,
      isLookingDownDetected: false,
    }
  },
  mounted() {
  ...
  methods: {
      ...
      return new Promise(async resolve => {
        if (typeof MediaRecorder === "function") {
          ...
          this.mediaRecorder.ondataavailable = e => {
            this.blob = new Blob([e.data], { 'type': e.currentTarget.mimeType });
          };
    ...
    saveVideo() {
      ...
      this.$dialog.confirm({
        title: '录制完成',
        message: `<video src="${url}#t=0.01" style="display: block;width: 100%;" webkit-playsinline="true" playsinline="true" controls autoplay></video>
    ...
    triggerAction(actionValue) {
      if (this.isCameraOpen && this.inProgress) {
        ...
        //两个动作间隔3s
        if (findObj.value === actionValue && deffTime >= 2000) {
    ...
    drawResults(faces, ctx) {
      faces.forEach(faceItem => {
        ...
        if (this.isCameraOpen && this.inProgress) {
          ...
          // 向上抬头
          this.isLookingUp(faceItem, ctx);

          // 向下低头
          this.isLookingDown(faceItem, ctx);
        }
      });
    },
    ...
    isOpenMouth(face, ctx) {
      if (this.isOtherActionDetected('openMouth')) return; // 检查其他动作
    ...
    //眨眼
    isWink(face, ctx) {
      if (this.isOtherActionDetected('wink')) return; // 检查其他动作
      ...
      if (leftProportion <= 5 || rightProportion <= 5) { // 调整阈值
        this.isWinkArr.push([leftProportion, rightProportion]);
        // 连续2帧一次
        if (this.isWinkArr.length >= 3) { // 将4帧改为2帧
          console.log('Detected wink'); // 调试输出
    ...
    isShakingHisHead(face, ctx) {
      ...
      this.isShakingHisHeadArr.push(diff);

      console.log(`Head turn diff: ${diff}`); // 调试输出

      // 计算4帧的动态变化
      if (this.isShakingHisHeadArr.length > 4) {
        this.isShakingHisHeadArr.shift();
        const isLeftTurn = this.isShakingHisHeadArr.every(e => this.flipHorizontal ? e <= -20 : e >= 20); // 调整阈值
        const isRightTurn = this.isShakingHisHeadArr.every(e => this.flipHorizontal ? e >= 20 : e <= -20); // 调整阈值
        if (isLeftTurn) {
          console.log('Detected left turn'); // 调试输出
          this.triggerAction(4); // 向左转
        }
        if (isRightTurn) {
          console.log('Detected right turn'); // 调试输出
          this.triggerAction(5); // 向右转
        }
      }
    },
    actionSelect(value) {
    ...
    isLookingUp(face, ctx) {
      ...
      const lips = face.keypoints.find(point => point.name === "lips");
      const rightEyebrow = face.keypoints.find(point => point.name === "rightEyebrow");
      const leftEyebrow = face.keypoints.find(point => point.name === "leftEyebrow");

      if (rightEye && leftEye && lips && rightEyebrow && leftEyebrow) {
        const eyeToEyebrowDiff = (rightEyebrow.y - rightEye.y + leftEyebrow.y - leftEye.y) / 2;
        const eyeToLipsDiff = (lips.y - rightEye.y + lips.y - leftEye.y) / 2;

        console.log(`Looking Up diffs: eyeToEyebrow=${eyeToEyebrowDiff}, eyeToLips=${eyeToLipsDiff}`); // 调试输出

        if (eyeToEyebrowDiff < 15 && eyeToLipsDiff > 5) { // 增大阈值
          console.log('Detected looking up'); // 调试输出
          this.triggerAction(6); // 向上抬头
        }
      }
    },
    // 向下低头
    isLookingDown(face, ctx) {
      ...
      if (rightEye && leftEye && lips && rightEyebrow && leftEyebrow) {
        ...
        console.log(`Looking Down diffs: eyeToEyebrow=${eyeToEyebrowDiff}, eyeToLips=${eyeToLipsDiff}`); // 调试输出

        if (eyeToEyebrowDiff < -10 && eyeToLipsDiff > 20) { // 增大阈值
          console.log('Detected looking down'); // 调试输出
          this.triggerAction(7); // 向下低头
        }
      }
    },
    isHeadTilted(face) {
      ...
      if (rightEye && rightEyebrow) {
        const diff = rightEyebrow.y - rightEye.y;
        console.log(`Head Tilt diff: ${diff}`); // 调试输出
        if (diff > 5) { // 根据实际情况调整阈值
          this.triggerAction(8); // 头部向右倾斜
        } else if (diff < -5) {
          this.triggerAction(9); // 部向左倾斜
        }
      }
    },
    isOtherActionDetected(currentAction) {
      switch (currentAction) {
        case 'wink':
          return this.isOpenMouthDetected || this.isShakingHeadDetected || this.isLookingUpDetected || this.isLookingDownDetected;
        case 'openMouth':
          return this.isWinkArr.length > 0 || this.isShakingHeadDetected || this.isLookingUpDetected || this.isLookingDownDetected;
